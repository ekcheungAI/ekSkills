...
function mountScrollWorld(container, config) {
  ...
  function loadClip(s) {
    ...
    s.loading = true;
    const gen = s.gen = (s.gen || 0);   // site patch: generation token, see unloadClip
    // Serve the lighter mobile encode on phones when one was provided.
    const url = (isMobile() && s.clipM) ? s.clipM : s.clip;
    fetch(url).then(r => r.ok ? r.blob() : Promise.reject(new Error('404')))
      .then(blob => {
        if (s.gen !== gen) return;      // unloaded while the fetch was in flight
  ...
  // site patch: iOS Safari only keeps roughly three media elements decodable at
  // once — a fourth blob video silently renders blank. On phones, release clips
  // once the camera is well past them (the still poster takes over) so the
  // active scene always gets a decoder slot; the clip reloads on approach
  // (hysteresis: load within 1.6vh, unload beyond 2.5vh, so no thrash).
  function unloadClip(s) {
    s.gen = (s.gen || 0) + 1;           // discard any in-flight load
    s.loading = false;
    if (!s.video) return;
    try { URL.revokeObjectURL(s.video.src); } catch (e) {}
    try { s.video.removeAttribute('src'); s.video.load(); } catch (e) {}
    s.video.remove();
    s.video = null; s.hasClip = false; s.ready = false; s.cur = 0;
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      else if (isMobile() && (s.hasClip || s.loading) && (y < s.start - 2.5 * vh || y > s.end + 2.5 * vh)) unloadClip(s);
      const local = clamp((y - s.start) / (s.end - s.start), 0, 1);
      s.target = s.linger ? lingerEase(local, s.linger) : local;
      let outside = 0;
      // site patch: hold the FINAL scene's last frame after its segment ends —
      // the content sheet slides over it instead of the world dissolving to empty sky
