...
|---|---|---|
| [`supergit`](skills/supergit/) | A proactive git steward for AI-agent projects: one read-only *library* per project, one *room* (worktree) per task, *ship* = branch + PR, only you merge. Plus a ledger-correct `migrate` for Supabase/Postgres. | The vocabulary (library / room / ship / merge / sync / park / rescue / TTL) and the authority tiers that let a non-technical owner direct any agent — Claude Code, Codex, Cursor — and always end with a clean state. Includes the eight agent rules to paste into any project. |
